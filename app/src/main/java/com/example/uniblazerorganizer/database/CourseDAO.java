...
public class CourseDAO {
    ...
    public List<Course> getCoursesByTermId(int termId) {
        List<Course> courses = new ArrayList<>();
        Cursor cursor = db.query("courses", null, "term_id = ?", new String[]{String.valueOf(termId)}, null, null, null);
    ...
    public int countCoursesByTermId(int termId) {
        int count = 0;
        Cursor cursor = db.rawQuery("SELECT COUNT(*) FROM courses WHERE term_id = ?", new String[]{String.valueOf(termId)});
